...
js là file javascript
.jsx là file javascript   mở rộng cú pháp cho react

2. Tại sao phải `export default App`?
     vì main jsx gọi import  default import mặc định, export default cho phép không cần import với {} va chi muốn mặc định thứ gửi đầu tiên từ app.jsx

3. Thử xóa `export default` → chuyện gì xảy ra?
    xóa export default thì web không chạy app.jsx không export public nên cac file khác không đọc được nội dung của app.jsx
...
- [v] Tạo project React chạy được
- [v] Hiểu file `.jsx` là gì
- [v] Viết được component cơ bản
- [v] Biết `class` → `className`, `for` → `htmlFor`
- [v] Biết phải đóng tất cả thẻ (`<img />`, `<input />`)
